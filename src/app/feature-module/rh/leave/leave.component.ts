  ...
  private loadStatusOptions(): void {
    this.leaveService.getStatusOptions().subscribe({
      next: (options) => {
        ...
        this.statusOptionsForForm = options.filter(opt => opt.value !== 'pending' && opt.value !== 'acceptedL1' && opt.value !== 'proposed_other_date');
        this.statusOptionsRH = options.filter(opt => opt.value !== 'proposed_other_date' && opt.value !== 'pending');
      },
      error: (err) => {
        console.error('Erreur lors du chargement :', err);
      }
